...
// On preview surfaces, broker the auth session to the editor over postMessage so
// preview instances share one login; otherwise fall back to localStorage.
export function brokeredPreviewStorage() {
  if (typeof window === "undefined") return undefined;
  const host = location.hostname;
  const PREVIEW_ZONES = ["preview.local", "localhost", "127.0.0.1"];
  ...
  // Post only to the real editor ancestor, validated as a trusted preview origin,
  // so the session token can never reach an untrusted embedder.
  const dev = host === "localhost" || host === "127.0.0.1";
  const EDITOR = dev
    ? /^https:\/\/([a-z0-9-]+\.)*localhost(:3000)?$|^http:\/\/localhost:3000$/

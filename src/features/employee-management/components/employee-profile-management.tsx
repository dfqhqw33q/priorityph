...
export function EmployeeProfileManagementPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Employee profile management" description="Manage employee profiles used for evaluation records. Access is limited to System Administrators." />
      <Card>
      ...
      {query.isLoading ? <LoadingBlock rows={5} /> : query.isError ? <EmptyState title="Employee profiles could not be loaded" description={query.error instanceof Error ? query.error.message : "Unavailable"} /> : rows.length === 0 ? <EmptyState title="No employee profiles" description="Create a profile before an employee uses the public evaluation portal." /> : (
        <div className="overflow-x-auto border border-border bg-card shadow-sm">
          <table className="w-full min-w-[760px] text-left text-sm"><caption className="sr-only">Employee profile management</caption><thead className="border-b border-primary/30 bg-primary text-primary-foreground dark:border-border dark:bg-muted/60 dark:text-foreground"><tr>{["Employee number", "Name", "Job title", "Division", "Section", "Status", "Action"].map((heading) => <th key={heading} className="px-4 py-3 font-semibold">{heading}</th>)}</tr></thead>

...
export const listTrainingData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let recommendations = admin
      ...
      .select(
        "id, employee_id, source_evaluation_id, training_title, related_competency, source, recommendation, status, created_at, employees!inner(full_name, employee_number, job_title, division, section), evaluations(evaluation_cycles(name, year))",
      )
      .order("created_at", { ascending: false });
    let records = admin
      .from("training_records")
      .select(
        "id, employee_id, source_evaluation_id, training_title, provider, training_date, status, related_competency, committee_recommendation, notes, created_at, employees!inner(full_name, employee_number, job_title, division, section), evaluations(evaluation_cycles(name, year))",
      )
      .order("created_at", { ascending: false });
    if (data.employeeId) {

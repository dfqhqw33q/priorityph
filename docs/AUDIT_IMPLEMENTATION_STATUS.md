...
## SECTION 6: AI INTEGRATION (OPENROUTER MINIMAX M3 FREE)

### 6.1 AI Provider and Configuration

| Component          | Current State                                                             | Evidence                                                             |
| ------------------ | ------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **Provider**       | OpenRouter MiniMax M3 Free                                                | [src/lib/ai-provider.server.ts](../src/lib/ai-provider.server.ts)    |
| **API Endpoint**   | `https://openrouter.ai/api/v1/chat/completions`                           | [src/lib/ai-provider.server.ts](../src/lib/ai-provider.server.ts)    |
| **Fallback**       | Explicit development mock only when `AI_PROVIDER=mock` outside production | [src/lib/ai-provider.server.ts](../src/lib/ai-provider.server.ts)    |
| **Environment**    | `OPENROUTER_API_KEY`, optional `OPENROUTER_MODEL`                         | Server-side only (no leakage to client)                              |
